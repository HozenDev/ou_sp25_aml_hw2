...
class FractionOfVarianceAccountedForSingle(FractionOfVarianceAccountedFor):
    '''
    FVAF = 1 - mse / var
    ...
    The original FVAF computes the metric for each output dim separately.  This
    one returns the average of these metrics across the output dims.
    
    '''
    
    def __init__(self, ndims, name='fvaf_single', **kwargs):
        '''
        @param ndims Number of network output dimensions (each dimension is treated
        separately in the FVAF computation)
        '''
        super(FractionOfVarianceAccountedForSingle, self).__init__(ndims=ndims, name=name, **kwargs)


    def result(self):
        '''
        @return Average FVAF across the output dimension (shape: 1)
        '''
        
        fvafs = super(FractionOfVarianceAccountedForSingle, self).result()
        
        return tf.reduce_mean(fvafs)
